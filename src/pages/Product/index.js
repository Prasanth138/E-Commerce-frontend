...
function Product({match, history}) {
    ...
    const submitReviewHandler = () => {
      ...
      setTimeout(()=>{history.push(`/product/${match.params.id}`)},1500) ;
    }
    ...
    return (
        <StyledContainer maxWidth={'lg'} >
            <Meta title={product.name ? `${product.name} | Book Store` : "Book Details | Book Store"} />
            {loading ? <Loader /> :
                error ? <Alert severity="error">{error}</Alert> :
                    <Grid container justify='space-between' spacing={3}>
                        ...
                        style={{backgroundColor:'#f9f9f9f9'}}>
                            <ImgAndInfo>
                            <img ref={ el => imgRef = el } src={`https://e-comerce-mern.herokuapp.com${product.image}`} alt={product.name} />
