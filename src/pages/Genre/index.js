...
const GenrePage = ({match}) => {
    ...
    useEffect(()=>{
        ...
        const fetchProducts = async() => {
            const {data} = await axios.get(`https://e-commerce-backend-aeuw.onrender.com/api/products/genre/${genre}`);
            setProducts(data);
            setLoading(false);
        };
        fetchProducts()
    ...
    return (
        <div style={{marginBottom: '30px'}}>
            <Meta title={`${genre.split('-').map(e => (e.charAt(0).toUpperCase() + e.slice(1))).join(' ')} Books | Book Store`} />
